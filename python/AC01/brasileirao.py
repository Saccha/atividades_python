...
def qtos_libertadores(dados):
    inf = dados['fases']['2700']['faixas-classificacao']['classifica1']['faixa']
    var = inf.split('-')
    var = int(var[1])
    return var
...
def ids_dos_melhor_classificados(dados,numero_de_times):
    lista_ids = []
    for a in range(numero_de_times):
        var = dados['fases']['2700']['classificacao']['grupo']['Único']
        lista_ids.append(var[a])
    return lista_ids    
...
def classificados_libertadores(dados):
    lista_classificados = []
    for b in range(qtos_libertadores(dados)):
        caminho = ids_dos_melhor_classificados(dados, qtos_libertadores(dados))
        lista_classificados.append(caminho[b])
...
def nomes_classificados_libertadores(dados):
    var = len(classificados_libertadores(dados))
    id = ids_dos_melhor_classificados(dados, var)
    lista_nomes = []
    for b in range(var):
        id2 = id[b]
        caminho = dados['equipes'][id2]['nome-comum']
        lista_nomes.append(caminho)
    return lista_nomes
...
'''
def ids_dos_times_de_um_jogo(dados,id_jogo):
    caminho = dados['fases']['2700']['jogos']['id'][id_jogo]
    return caminho['time1'], caminho['time2']
...
def nomes_dos_times_de_um_jogo(dados,id_jogo):
    t1 = ids_dos_times_de_um_jogo(dados,id_jogo)[0]
    t2 = ids_dos_times_de_um_jogo(dados,id_jogo)[1]
    nome1 = nome_do_time(dados,t1)
    nome2 = nome_do_time(dados,t2)
    return nome1, nome2
...
def id_do_time(dados,nome_time):
    for cont in dados['equipes']:
        verificar = dados['equipes'][cont]['nome-comum']
        if verificar == nome_time:
            return dados['equipes'][cont]['id']
...
def dicionario_id_estadio_e_nro_jogos(dados):
    ...
    for contador in jogos:
        dicionario_de_jogos[jogos[contador]['estadio_id']] = 0
    for contador in jogos:
        dicionario_de_jogos[jogos[contador]['estadio_id']] += 1
    return dicionario_de_jogos
...
def busca_imprecisa_por_nome_de_time(dados,nome_time):
    ...
    times = dados['equipes']
    for id in times:
        if (nome_time in times[id]['nome-comum'] or
            nome_time in times[id]['nome-slug'] or
            nome_time in times[id]['sigla'] or
                nome_time in times[id]['nome']):

            lista_times.append(id)
    return lista_times
...
def ids_de_jogos_de_um_time(dados,time_id):
    lista_de_jogos_de_um_time = []
    partidas = dados['fases']['2700']['jogos']['id']
    for id in partidas:
        if (time_id == partidas[id]['time1'] or
                time_id == partidas[id]['time2']):
            lista_de_jogos_de_um_time.append(id)
    return lista_de_jogos_de_um_time
...
def datas_de_jogos_de_um_time(dados,nome_time):
    lista_de_jogos_de_um_time = []
    time = id_do_time(dados, nome_time)
    jogos = dados['fases']['2700']['jogos']['id']
    for partida in jogos:
        if (time == jogos[partida]['time1'] or
                time == jogos[partida]['time2']):
            lista_de_jogos_de_um_time.append(jogos[partida]['data'])
    return lista_de_jogos_de_um_time
...
def dicionario_de_gols(dados):
    dicionario_de_gols = {}
    partidas = dados['fases']['2700']['jogos']['id']
    for id_jogos in partidas:
        if partidas[id_jogos]['time1'] not in dicionario_de_gols:
            dicionario_de_gols[partidas[id_jogos]['time1']] = 1
        else:
            dicionario_de_gols[partidas[id_jogos]['time1']
                ] += int(partidas[id_jogos]['placar1'])
        if partidas[id_jogos]['time2'] not in dicionario_de_gols:
            dicionario_de_gols[partidas[id_jogos]['time2']] = 1
        else:
            dicionario_de_gols[partidas[id_jogos]['time2']
                ] += int(partidas[id_jogos]['placar2'])
    return dicionario_de_gols
...
def time_que_fez_mais_gols(dados):
    ...
    idTimes = list(times.keys())
    golsTimes = list(times.values())
    for gols in times:
        if times[gols] > qtd_maior_de_gols:
            qtd_maior_de_gols = times[gols]

    return idTimes[golsTimes.index(qtd_maior_de_gols)]
...
def rebaixados(dados):
    lista_dos_times_rebaixados = []
    classificacao = dados['fases']['2700']['classificacao']['grupo']['Único']
    zona_de_rebaixamento = dados['fases']['2700']['faixas-classificacao']['classifica3']['faixa']
    inicio = zona_de_rebaixamento.split('-')[0]
    inicio = int(inicio)
    fim = zona_de_rebaixamento.split('-')[1]
    fim = int(fim)
    for time in range(inicio-1, fim):
        lista_dos_times_rebaixados.append(classificacao[time])
    return lista_dos_times_rebaixados
...
def classificacao_do_time_por_id(dados,time_id):
    classificacao_do_time_por_id = dados['fases']['2700']['classificacao']['grupo']['Único']
    qtd_times = len(classificacao_do_time_por_id)
    for posicao in range(qtd_times):
        if classificacao_do_time_por_id[posicao] == time_id:
            return posicao+1
    return 'nao encontrado'
